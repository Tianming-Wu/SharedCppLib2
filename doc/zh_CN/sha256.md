...
// ... 任意多次
scl2::bytearray digest = hasher.end();  // 32 字节摘要
```

### 从 istream 流式处理

```cpp
#include <SharedCppLib2/hash_api.hpp>  // 提供 hash_stream
...
这些常量可被 `scl2::hmac<HashProvider>` 等兼容层以及 `scl2::generic_buffer_size<sha256>()` 使用。
